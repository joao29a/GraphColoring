...
Graph* tabu_search(Graph* graph, int tabu_len, int candidates_len, int max_iter,
    ...
    Graph** best_threads = (Graph**) malloc(sizeof(Graph*) * num_threads);

    #pragma omp parallel
    {
        int thread_num = omp_get_thread_num();
        best_threads[thread_num] = initial_solution(graph);
        int best_cost_thread = cost(best_threads[thread_num]);

        srand(time(NULL));

        int new_best_cost = best_cost_thread - 1;
        Graph* s = reduce_color(best_threads[thread_num], new_best_cost);
        ...
        for (int i = 0; i < max_iter; i++){
            ...
            Graph** candidates = (Graph**) malloc(sizeof(Graph*) * 
                    (candidates_len / num_threads));

            if (conflict_c > 0){
                for (int i = 0; i < candidates_len / num_threads; i++){
                    candidates[i] = generate_candidates(s, conflict_vt, conflict_c,
                            tabu_list, &tabu_pos, new_best_cost);
                }

                int conflict_cand_c;
                Graph* best_candidate = get_best_candidate(candidates, 
                        candidates_len / num_threads, conflict_vt, conflict_c, 
                        &conflict_cand_c);
            ...
            if (conflict_c == 0){
                best_cost_thread = new_best_cost;
                free_graph(best_threads[thread_num]);
                best_threads[thread_num] = s;

                new_best_cost = best_cost_thread - 1;
                s = reduce_color(best_threads[thread_num], new_best_cost);
                free(conflict_vt);
                conflict_vt = conflict_vertices(s, &conflict_c);
            }
        ...
        #pragma omp critical
        {
            if (best_cost_thread < best_cost){
                best_cost = best_cost_thread;
                if (best != NULL) free(best);
                best = best_threads[thread_num];
            }
            else free(best_threads[thread_num]);
        }
